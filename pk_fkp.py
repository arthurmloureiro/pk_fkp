...
if realiz_type == 1:
	...
	for m in range(num_realiz):
		...
		#                  FKP                   #
		##########################################
		k,P,sigma = ext.FKP(N_r,num_bins,n_bar,bias,cell_size,n_x)
...
pl.figure('P(k)')
pl.plot(k_camb,Pk_camb,label='CAMB',linewidth=1.5)
pl.errorbar(k,P,yerr=sigma,label='Estimated P(k)')
pl.xlabel(r'$k$ [h/Mpc]',fontsize=25)
pl.xlabel(r'$P(k)$ [Mpc/h]$^3$',fontsize=25)
pl.grid(True)
pl.axvline(x=k_max/2.,color='k',label='Nyquist f.')
pl.loglog()
pl.legend()
pl.show()
